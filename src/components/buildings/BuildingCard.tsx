...
export default function BuildingCard({ building, onDelete }: Props) {
  const badgeClass = TYPE_BADGE_COLORS[building.type] || 'bg-gray-600/20 text-gray-400';

  return (
    <div className="bg-[#111118] border border-gray-800 rounded-xl p-5 hover:border-gray-700 transition-colors group">
      {/* Icon placeholder */}
      <div className="w-full h-28 bg-[#0a0a12] rounded-lg mb-4 flex items-center justify-center">
      ...
      <p className="text-xs text-gray-500 mb-4 truncate">{building.address || 'No address'}</p>

      <div className="flex gap-2">
        <Link
          href={`/buildings/${building.id}/edit`}
          className="flex-1 text-center text-xs font-medium px-3 py-2 rounded-lg bg-blue-600/15 text-blue-400 hover:bg-blue-600/25 transition-colors"
        >
          Edit
        </Link>
        <button
          ...
          className="text-xs font-medium px-3 py-2 rounded-lg bg-red-600/10 text-red-400 hover:bg-red-600/20 transition-colors"
        >
          Delete
        </button>
      </div>
